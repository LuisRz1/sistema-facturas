...
@section('content')
    ...
    <div class="page-header">
        <div>
            ...
            <h1 class="page-title" style="font-size:22px;">
                {{ $cotizacion->tipo_cotizacion === 'MAQUINARIA' ? '' : '' }}
            ...
            <table class="row-table" id="rowTable">
                ...
                @if($filas->count() > 0)
                    <tr class="sum-row" id="sumRow">
                        <td colspan="{{ $esMaquinaria ? 12 : 10 }}" style="text-align:right;font-size:12px;letter-spacing:.4px;text-transform:uppercase;">
                            TOTAL
                        </td>
                ...
                <form id="addRowForm" enctype="multipart/form-data" onsubmit="addRow(event)">
                    ...
                    @if($esMaquinaria)
                        ...
                        <div style="display:grid;grid-template-columns:140px 1fr 1fr 1fr 1fr 80px 80px 80px 80px 90px 90px;gap:8px;margin-bottom:10px;">
                            <div>
                                ...
                                <input type="date" class="row-input" id="rFecha" name="fecha"
                            ...
                            <div>
                                ...
                                <select class="row-input" id="rChofer" name="id_chofer" required>
                                    ...
                                    @foreach($choferes as $ch)
                                        <option value="{{ $ch->id_chofer }}">{{ $ch->nombres }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input type="text" class="row-input" id="rObra" name="obra_maquina"
                                       value="{{ $cotizacion->obra }}" placeholder="Obra">
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input type="number" class="row-input" id="rHI" name="hora_inicio" step="0.01"
                                       placeholder="2916.8" required
                                       onblur="onHIBlur()" oninput="calcTotalFila()">
                            </div>
                            <div>
                                <div class="row-input-lbl">HT *</div>
                                <input type="number" class="row-input" id="rHT" name="hora_fin" step="0.01"
                                       placeholder="2917.65" required oninput="calcTotalFila()">
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input type="number" class="row-input" id="rHMin" name="hora_minima" step="0.01"
                                       value="3" required oninput="calcTotalFila()">
                            </div>
                            <div>
                                <div class="row-input-lbl">Precio/H *</div>
                                <input type="number" class="row-input" id="rPrecio" name="precio_hora" step="0.01"
                                       placeholder="245.00" required oninput="calcTotalFila()">
                            </div>
                            <div>
                        ...
                        <div style="display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:8px;align-items:end;">
                            ...
                            <div>
                                ...
                                <div style="display:flex;align-items:center;gap:8px;">
                                    <button type="button" class="file-btn" id="btnImgParte"
                                            onclick="document.getElementById('inputImgParte').click()">
                                        📷 Adjuntar imagen
                                    </button>
                                    <input type="file" id="inputImgParte" name="imagen_parte_diario"
                                           accept="image/*" style="display:none;"
                                           onchange="onFileSelected(this,'btnImgParte','📷')">
                                </div>
                            </div>
                            <div></div>
                            <div>
                                <button type="submit" class="btn btn-primary" id="btnAddRow">
                                    + Agregar Fila
                                </button>
                            </div>
                        </div>
                    ...
                    @else
                        ...
                        <div style="display:grid;grid-template-columns:140px 1fr 1fr 1fr 1fr 80px 90px 90px;gap:8px;margin-bottom:10px;">
                            ...
                            <div>
                                <div class="row-input-lbl">Obra</div>
                                <input type="text" class="row-input" id="rObra" name="obra_agregado"
                                       value="{{ $cotizacion->obra }}" placeholder="Obra">
                            </div>
                            <div>
                                <div class="row-input-lbl">Placa</div>
                            ...
                            <div>
                                ...
                                <input type="number" class="row-input" id="rM3" name="m3" step="0.01"
                                       placeholder="24" required oninput="calcTotalFila()">
                            </div>
                            <div>
                                <div class="row-input-lbl">Precio/M³ *</div>
                                <input type="number" class="row-input" id="rPrecioM3" name="precio_m3" step="0.01"
                                       placeholder="300.00" required oninput="calcTotalFila()">
                            </div>
                            <div>
                                <div class="row-input-lbl">Total Fila</div>
                                <input type="number" class="row-input calculated" id="rTotal" step="0.01" readonly placeholder="0.00">
                        ...
                        <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr auto;gap:8px;align-items:end;">
                            ...
                            <div>
                                ...
                                <button type="button" class="file-btn" id="btnPdfGrr"
                                        onclick="document.getElementById('inputPdfGrr').click()">
                                    📄 Adjuntar PDF
                                </button>
                                <input type="file" id="inputPdfGrr" name="archivo_grr"
                                       accept="application/pdf" style="display:none;"
                                       onchange="onFileSelected(this,'btnPdfGrr','📄')">
                            </div>
                            <div>
                                <button type="submit" class="btn btn-primary" id="btnAddRow">
                                    + Agregar Fila
            ...
            <div id="phantomAlert" style="display:none;margin-top:14px;background:#fef3c7;border:2px dashed #fbbf24;border-radius:10px;padding:14px 18px;">
                <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
                    ...
                    <div style="display:flex;gap:8px;">
                        <button class="btn" style="background:#fbbf24;color:#78350f;font-size:12px;padding:7px 14px;"
                                id="btnAceptarPhantom" onclick="aceptarPhantom()">
                            ✓ Agregar fila fantasma
                        </button>
                        <button class="btn btn-ghost" style="font-size:12px;padding:7px 14px;"
                                onclick="document.getElementById('phantomAlert').style.display='none';">
                            Ignorar
                        </button>
                    </div>
                </div>
    ...
    <div class="modal-overlay" id="modalDelFila">
        <div class="modal" style="max-width:400px;">
            <div class="modal-header" style="background:#7f1d1d;">
                ...
                <button onclick="document.getElementById('modalDelFila').classList.remove('open')"
    ...
    <div class="modal-overlay" id="modalEditFila">
        <div class="modal" style="max-width:720px;">
            <div class="modal-header">
                ...
                <button onclick="document.getElementById('modalEditFila').classList.remove('open')"
                        style="position:absolute;right:20px;top:20px;background:none;border:none;color:#fff;cursor:pointer;font-size:24px;">×</button>
            </div>
    ...
    <div class="modal-overlay" id="modalEditHeader">
        <div class="modal" style="max-width:640px;">
            <div class="modal-header">
                ...
                <button onclick="document.getElementById('modalEditHeader').classList.remove('open')"
                        style="position:absolute;right:20px;top:20px;background:none;border:none;color:#fff;cursor:pointer;font-size:24px;">×</button>
            </div>
                ...
                <div class="modal-body" style="padding:24px;">
                    ...
                    <div class="field-row cols2" style="margin-bottom:14px;">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="numero_valorizacion" class="form-input" id="editHeaderNum"
                    ...
                    <div class="field-row cols2">
                        <div class="form-group">
                            ...
                            <input type="date" name="periodo_inicio" class="form-input" id="editHeaderDesde"
                                   value="{{ $cotizacion->periodo_inicio }}">
                        </div>
                        <div class="form-group">
                            <label class="form-label">Período Fin *</label>
                            <input type="date" name="periodo_fin" class="form-input" id="editHeaderHasta"
                                   value="{{ $cotizacion->periodo_fin }}">
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
    ...
    <script>
        ...
        const CHOFERES  = @json($choferes->map(fn($c) => ['id' => $c->id_chofer, 'nombre' => $c->nombres]));
        const MAQUINARIAS = @json($maquinarias->map(fn($m) => ['id' => $m->id_maquinaria, 'nombre' => $m->nombre]));
        const AGREGADOS = @json($agregados->map(fn($a) => ['id' => $a->id_agregado, 'nombre' => $a->nombre]));
    ...
    <script>
        ...
        function onFileSelected(input, btnId, icon) {
            ...
            if (input.files && input.files.length > 0) {
                const name = input.files[0].name.substring(0, 20);
        ...
        // ── Phantom row detection (MAQUINARIA only) ────────────────────────────────
        ...
        function onHIBlur() {
            ...
            // HT is column index 8 (0-based) in maquinaria
            const cells = lastRow.querySelectorAll('td');
            // HT (hora_fin) is at index 9 in the rendered row (after #,fecha,chofer,maqu,placa,obra,nparte,hi,ht,htrab,hmin,prec,total,acc)
            const htValue = parseFloat(cells[8]?.textContent) || 0;
            const hiValue = parseFloat(document.getElementById('rHI')?.value) || 0;
            const prevFecha = cells[1]?.textContent?.trim() || '';
            const currFecha = document.getElementById('rFecha')?.value || '';
            ...
            if (htValue > 0 && hiValue > htValue + 0.05) {
                ...
                const desc = `Horómetro previo terminó en ${htValue.toFixed(1)} y el actual inicia en ${hiValue.toFixed(1)} — Gap: ${gap} hrs`;
                document.getElementById('phantomDesc').textContent = desc;
        ...
        async function aceptarPhantom() {
            ...
            // Collect current row data as the phantom row
            ...
            if (!chofer || !maq) {
                showToast('Completa Chofer y Maquinaria antes de aceptar la fila fantasma.', false);
                return;
            }

            fd.append('fecha',       fecha);
            fd.append('id_chofer',   chofer);
            fd.append('id_maquinaria', maq);
            fd.append('obra_maquina',  obra);
            fd.append('placa',         placa);
            fd.append('hora_inicio',   phantomData.hora_inicio);
            fd.append('hora_fin',      phantomData.hora_fin);
            fd.append('hora_minima',   hmin);
            fd.append('precio_hora',   precio);
            fd.append('n_parte_diario','');
        ...
        function appendRowToTable(row) {
            ...
            const html  = buildRowHtml(row, idx);

            // Remove sum row, add new row, re-add sum row
        ...
        function buildRowHtml(row, idx) {
            if (ES_MAQUINARIA) {
                const maqNombre  = MAQUINARIAS.find(m => m.id == row.id_maquinaria)?.nombre || '—';
        ...
        function updateSumRow() {
            ...
            const existing = document.getElementById('sumRow');
        ...
        function resetAddForm() {
            if (ES_MAQUINARIA) {
                ['rHI','rHT','rHTrab','rTotal'].forEach(id => {
                ...
                ['rM3','rTotal','rNParte','rGRR'].forEach(id => {
                    const el = document.getElementById(id);
                    if (el) el.value = '';
                });
                ['inputImgParte','inputPdfGrr'].forEach(id => {
                    const el = document.getElementById(id);
                    if (el) el.value = '';
                });
                ['btnImgParte','btnPdfGrr'].forEach((id, i) => {
                    const btn = document.getElementById(id);
                    if (btn) { btn.textContent = i===0?'📷 Adjuntar':'📄 Adjuntar PDF'; btn.classList.remove('uploaded'); }
                });
            }
        }
        ...
        document.getElementById('btnConfDelFila').addEventListener('click', async () => {
            ...
            if (data.success) {
                const row = document.querySelector(`#rowTbody tr[data-id="${deleteRowId}"]`);
                if (row) row.remove();
                actualizarTotales(data.totales);
                // Renumber rows
        ...
        function abrirEditarFila(rowId) {
            ...
            const body = document.getElementById('editFilaBody');
            body.innerHTML = buildEditForm(row);
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:14px;">
            <div class="form-group"><label class="form-label">Fecha</label>
                <input type="date" name="fecha" class="form-input" value="${r.fecha||''}" required></div>
            <div class="form-group"><label class="form-label">Chofer</label>
                <select name="id_chofer" class="form-input" required><option value="">—</option>${choferOpts}</select></div>
            <div class="form-group"><label class="form-label">Maquinaria</label>
                <select name="id_maquinaria" class="form-input" required><option value="">—</option>${maqOpts}</select></div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px;">
            <div class="form-group"><label class="form-label">Obra</label>
                <input type="text" name="obra_maquina" class="form-input" value="${r.obra_maquina||''}"></div>
            <div class="form-group"><label class="form-label">Placa</label>
                <input type="text" name="placa" class="form-input" value="${r.placa||''}"></div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr 1fr;gap:10px;margin-bottom:14px;">
            <div class="form-group"><label class="form-label">HI *</label>
                <input type="number" name="hora_inicio" class="form-input" step="0.01" value="${r.hora_inicio||''}" oninput="calcEditTotal()" required></div>
            <div class="form-group"><label class="form-label">HT *</label>
                <input type="number" name="hora_fin" class="form-input" step="0.01" value="${r.hora_fin||''}" oninput="calcEditTotal()" required></div>
            <div class="form-group"><label class="form-label">H.Trab.</label>
                <input type="number" class="form-input" style="background:#f8fafc;" id="eHTrab" step="0.01" readonly value="${r.horas_trabajadas||''}"></div>
            <div class="form-group"><label class="form-label">H.Mín. *</label>
                <input type="number" name="hora_minima" class="form-input" step="0.01" value="${r.hora_minima||3}" oninput="calcEditTotal()" required></div>
            <div class="form-group"><label class="form-label">Precio/H *</label>
                <input type="number" name="precio_hora" class="form-input" step="0.01" value="${r.precio_hora||''}" oninput="calcEditTotal()" required></div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
            <div class="form-group"><label class="form-label">N° Parte Diario</label>
                <input type="text" name="n_parte_diario" class="form-input" value="${r.n_parte_diario||''}"></div>
            <div class="form-group"><label class="form-label">Imagen Parte Diario</label>
                <input type="file" name="imagen_parte_diario" class="form-input" accept="image/*" style="height:auto;padding:6px 10px;"></div>
        </div>`;
            } else {
                return `
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:14px;">
            ...
            <div class="form-group"><label class="form-label">Chofer</label>
                <select name="id_chofer" class="form-input" required><option value="">—</option>${choferOpts}</select></div>
            <div class="form-group"><label class="form-label">Agregado</label>
                <select name="id_agregado" class="form-input" required><option value="">—</option>${agrOpts}</select></div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px;">
            <div class="form-group"><label class="form-label">Obra</label>
                <input type="text" name="obra_agregado" class="form-input" value="${r.obra_agregado||''}"></div>
            <div class="form-group"><label class="form-label">Placa</label>
                <input type="text" name="placa" class="form-input" value="${r.placa||''}"></div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px;">
            <div class="form-group"><label class="form-label">M³ *</label>
                <input type="number" name="m3" class="form-input" step="0.01" value="${r.m3||''}" oninput="calcEditTotal()" required></div>
            <div class="form-group"><label class="form-label">Precio/M³ *</label>
                <input type="number" name="precio_m3" class="form-input" step="0.01" value="${r.precio_m3||''}" oninput="calcEditTotal()" required></div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;">
            <div class="form-group"><label class="form-label">N° Parte Diario</label>
                <input type="text" name="n_parte_diario" class="form-input" value="${r.n_parte_diario||''}"></div>
            <div class="form-group"><label class="form-label">N° GRR</label>
                <input type="text" name="grr" class="form-input" value="${r.grr||''}"></div>
            <div class="form-group"><label class="form-label">Imagen Parte</label>
                <input type="file" name="imagen_parte_diario" class="form-input" accept="image/*" style="height:auto;padding:6px 10px;"></div>
        </div>
        ...
        function abrirEditarHeader() {
            document.getElementById('modalEditHeader').classList.add('open');
        }
