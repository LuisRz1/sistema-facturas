            ...
            <table id="facturasTable">
                ...
                @forelse($facturas as $factura)
                    ...
                    <tr data-cliente="{{ $factura->id_cliente }}" data-estado="{{ $factura->estado }}"
                        ...
                        <td style="text-align:center;font-weight:600;">{{ $porcentaje > 0 ? $porcentaje.'%' : '—' }}</td>
                        ...
                        <td style="text-align:center;font-size:10px;font-weight:600;color:#7c3aed;">{{ $tipoRecaudacion ?? '—' }}</td>
                        ...
                        <td>
                            <div class="actions-cell" style="justify-content:flex-end;flex-wrap:wrap;gap:4px;">
                                <button type="button" onclick="abrirModalEditar('{{ $factura->id_factura }}')"
                                        class="action-btn" title="Editar" style="color:#7c3aed;">
                                ...
                                <button type="button" onclick="abrirModalComprobante('{{ $factura->id_factura }}')"
                                        class="action-btn" title="Subir imagen"
                                ...
                                @if($puedeNotificarDeuda)
                                    <form method="POST" action="{{ route('facturas.enviar-whatsapp-manual',$factura->id_factura) }}" style="display:inline;">@csrf
                                        <button type="submit" class="btn-icon-text btn-wa" title="WA deuda">
                                    ...
                                    <form method="POST" action="{{ route('facturas.enviar-correo-manual',$factura->id_factura) }}" style="display:inline;">@csrf
                                        <button type="submit" class="btn-icon-text btn-mail" title="Correo deuda">
                                ...
                                @if($factura->estado === 'PAGADA')
                                    <form method="POST" action="{{ route('facturas.enviar-factura-pagada-whatsapp',$factura->id_factura) }}" style="display:inline;">@csrf
                                        <button type="submit" class="btn-icon-text btn-wa" style="background:{{ $tieneComprobante?'#a7f3d0':'#d1fae5' }};" title="Confirmación pagada WA">
                                    ...
                                    <form method="POST" action="{{ route('facturas.enviar-factura-pagada-correo',$factura->id_factura) }}" style="display:inline;">@csrf
                                        <button type="submit" class="btn-icon-text btn-mail" style="background:#bfdbfe;" title="Confirmación pagada Correo">
                                            <svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                                        </button>
                                    </form>
                                @endif
        ...
        <div class="modal">
            ...
            <form id="formComprobante" onsubmit="enviarComprobante(event)" enctype="multipart/form-data">
                ...
                <div class="modal-body" style="text-align:center;padding:32px 24px;">
                    <div id="dropZone" style="border:2px dashed #cbd5e1;border-radius:12px;padding:36px;cursor:pointer;transition:all .2s;"
                        ...
                        <svg width="48" height="48" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5" style="margin:0 auto;color:#cbd5e1;"><path stroke-linecap="round" stroke-linejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
    ...
    @push('scripts')
        <script>
            ...
            document.getElementById('dropZone').addEventListener('drop', e => {
                e.preventDefault();
                if (e.dataTransfer.files.length) { document.getElementById('fileComprobante').files = e.dataTransfer.files; mostrarPreview({target:{files:e.dataTransfer.files}}); }
            });
            ...
            function guardarFactura(event) {
                ...
                fetch(`/facturas/${facturaActualId}`,{method:'PUT',headers:{'Content-Type':'application/json','X-Requested-With':'XMLHttpRequest','X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').content},body:JSON.stringify(datos)})
                    ...
                    .then(data=>{if(data.success){cerrarModalEditar();location.reload();}else alert('Error: '+(data.message||'No se pudo guardar'));})
            ...
            function mostrarPreview(event) {
                ...
                document.getElementById('preview').style.display='block'; document.getElementById('dropZone').style.display='none';
                if(file.type==='application/pdf'){document.getElementById('previewImg').style.display='none';document.getElementById('previewPdf').style.display='block';}
                else{const r=new FileReader();r.onload=e=>{document.getElementById('previewImg').src=e.target.result;document.getElementById('previewImg').style.display='block';document.getElementById('previewPdf').style.display='none';};r.readAsDataURL(file);}
            }
            function limpiarPreview() { document.getElementById('fileComprobante').value='';document.getElementById('preview').style.display='none';document.getElementById('dropZone').style.display='block';document.getElementById('previewImg').src='';document.getElementById('btnEnviarComprobante').disabled=false;document.getElementById('btnEnviarComprobante').textContent='☁️ Subir Imagen'; }
            function enviarComprobante(event) {
                event.preventDefault();
                const file=document.getElementById('fileComprobante').files[0];if(!file){alert('Por favor selecciona un archivo');return;}
                ...
                fetch(`/facturas/${facturaActualId}/upload-comprobante`,{method:'POST',body:new FormData(document.getElementById('formComprobante')),headers:{'X-Requested-With':'XMLHttpRequest'}})
                    .then(r=>r.json()).then(data=>{clearInterval(iv);document.getElementById('progressBar').style.width='100%';setTimeout(()=>{if(data.success){cerrarModalComprobante();location.reload();}else{alert(data.error||'Error al subir');btn.disabled=false;btn.textContent='☁️ Subir Imagen';document.getElementById('uploadProgress').style.display='none';}},400);})
                    .catch(err=>{clearInterval(iv);alert('Error: '+err.message);btn.disabled=false;btn.textContent='☁️ Subir Imagen';document.getElementById('uploadProgress').style.display='none';});
            }
            ...
            function filtrarTabla() {
                ...
                document.querySelectorAll('#facturasBody tr[data-estado]').forEach(row=>{
                    const ok=(!search||row.dataset.search.includes(search))&&(!estado||row.dataset.estado===estado)&&(!moneda||row.dataset.moneda===moneda)&&(!empresa||row.dataset.cliente===empresa);
