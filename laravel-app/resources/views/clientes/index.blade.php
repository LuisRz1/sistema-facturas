{{-- REEMPLAZAR: laravel-app/resources/views/clientes/index.blade.php --}}
{{-- CAMBIOS APLICADOS:
     1. Corregido conteo de estados: estado_contacto → estado_contado
     2. Corregido badge class: estado_contacto → estado_contado
     3. Corregido data-estado: estado_contacto → estado_contado
     4. Agregado botón "Importar Excel" en page-header
--}}
@extends('layouts.app')
...
@push('styles')
    ...
    <style>
        ...
        .page-header {
            ...
            padding: 28px 32px 0; animation: fadeDown .5s cubic-bezier(.16,1,.3,1) both;
        }
        .accent-bar { display: inline-block; width: 28px; height: 3px; background: var(--gold); border-radius: 2px; margin-right: 8px; vertical-align: middle; margin-bottom: 3px; }
        ...
        .stat-chips { display: flex; flex-wrap: wrap; gap: 10px; padding: 20px 32px 0; animation: fadeDown .5s .1s cubic-bezier(.16,1,.3,1) both; }
        .chip { display: flex; align-items: center; gap: 10px; background: var(--white); border: 1px solid var(--gold-b); border-radius: 14px; padding: 11px 18px; cursor: default; transition: transform .15s, box-shadow .15s; animation: chipPop .4s cubic-bezier(.34,1.56,.64,1) both; }
        .chip:hover { transform: translateY(-2px); box-shadow: 0 6px 22px #f5c84222; }
        .chip:nth-child(1) { animation-delay: .1s } .chip:nth-child(2) { animation-delay: .17s } .chip:nth-child(3) { animation-delay: .24s } .chip:nth-child(4) { animation-delay: .31s }
        .chip-icon { width: 34px; height: 34px; border-radius: 10px; background: var(--gold-l); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        ...
        .card { background: var(--white) !important; border: 1px solid var(--gold-b) !important; border-radius: 20px !important; margin: 20px 32px !important; overflow: hidden; animation: slideUp .55s .15s cubic-bezier(.16,1,.3,1) both; }
        ...
        th { padding: 10px 16px !important; font-size: 10.5px !important; font-weight: 700 !important; letter-spacing: .7px !important; text-transform: uppercase !important; color: var(--text-s) !important; white-space: nowrap; border: none !important; }
        tbody tr { border-bottom: 1px solid #fef9e0 !important; transition: background .15s; animation: rowIn .4s cubic-bezier(.16,1,.3,1) both; }
        ...
        .avatar-circle { width: 38px; height: 38px; border-radius: 50%; background: var(--gold-l) !important; border: 2px solid var(--gold-b); color: var(--gold-d) !important; display: flex; align-items: center; justify-content: center; font-weight: 800 !important; font-size: 14px !important; flex-shrink: 0; transition: transform .2s, box-shadow .2s; }
        tbody tr:hover .avatar-circle { transform: scale(1.1); box-shadow: 0 4px 12px #f5c84230; }
        .client-row-main { display: flex; align-items: center; gap: 11px; }
        .client-name  { font-weight: 700; font-size: 13.5px; color: var(--text); }
        .client-since { font-size: 11px; color: var(--text-s); margin-top: 1px; }
        .ruc-tag { display: inline-block; background: var(--gold-l); border: 1px solid var(--gold-b); border-radius: 6px; padding: 3px 9px; font-size: 12px; font-weight: 700; color: var(--gold-xd); letter-spacing: .3px; font-family: 'Courier New', monospace; }
        .contact-item { display: flex; align-items: center; gap: 5px; font-size: 12px; color: var(--text-m) !important; margin-bottom: 3px; }
        .contact-item svg { color: var(--gold-m) !important; flex-shrink: 0; }
        .contact-item:last-child { margin-bottom: 0; }
        .estado-badge { display: inline-flex; align-items: center; gap: 5px; padding: 3px 10px; border-radius: 50px; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: .4px; }
        .estado-badge::before { content: ''; width: 5px; height: 5px; border-radius: 50%; background: currentColor; opacity: .7; flex-shrink: 0; }
        ...
        .tipo-badge { display: inline-block; padding: 2px 8px; border-radius: 5px; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: .3px; }
        .tipo-juridica { background: #dbeafe; color: #1d4ed8; }
        .tipo-natural  { background: #f3e8ff; color: #7e22ce; }
        .actions-cell { display: flex; align-items: center; gap: 4px; justify-content: flex-end; }
        .action-btn { width: 32px; height: 32px; border-radius: 8px; border: 1px solid var(--gold-b) !important; background: var(--white) !important; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all .15s; color: var(--text-m) !important; }
        .action-btn:hover { background: var(--gold-l) !important; border-color: var(--gold-m) !important; color: var(--gold-d) !important; transform: scale(1.1); }
        .action-btn svg { width: 13px; height: 13px; }
        .empty-state { text-align: center; padding: 56px 24px; color: var(--text-s); }
        .empty-state svg { color: var(--gold-b); margin: 0 auto 16px; display: block; }
        .modal-overlay { position: fixed; inset: 0; background: rgba(28,22,0,.35); display: flex; align-items: center; justify-content: center; z-index: 50; opacity: 0; pointer-events: none; transition: opacity .25s; padding: 24px; }
        .modal-overlay.open { opacity: 1; pointer-events: all; }
        .modal { background: var(--white); border-radius: 22px; border: 1px solid var(--gold-b); width: 100%; max-width: 520px; transform: translateY(24px) scale(.97); transition: transform .3s cubic-bezier(.16,1,.3,1); overflow: hidden; }
        .modal-overlay.open .modal { transform: translateY(0) scale(1); }
        .modal-header { background: var(--gold-l); padding: 22px 28px 18px; border-bottom: 1px solid var(--gold-b); position: relative; }
        ...
        .modal-header h2 { font-size: 16px !important; font-weight: 800 !important; color: var(--text) !important; }
        ...
        @media (max-width: 768px) { .page-header, .stat-chips, .card { padding: 16px !important; margin: 12px !important; } .form-grid { grid-template-columns: 1fr; } .form-full { grid-column: 1; } }
    </style>
@endpush
...
@section('content')
    ...
    @if(session('resumen_importacion'))
        ...
        <div style="display:flex;align-items:center;gap:14px;background:#d1fae5;border:1.5px solid #6ee7b7;border-radius:12px;padding:14px 20px;margin:20px 32px 0;animation:slideUp .4s ease-out;">
            ...
            <div style="flex:1;">
                <strong style="color:#065f46;font-size:13px;">
                    ✓ Importación completada — {{ $ri['insertados'] }} cliente(s) nuevo(s), {{ $ri['actualizados'] }} actualizado(s)
                </strong>
                @if($ri['omitidos'] > 0)
                    <div style="font-size:11px;color:#047857;margin-top:2px;">{{ $ri['omitidos'] }} filas omitidas (sin número o denominación)</div>
                @endif
            </div>
        </div>
    @endif
    ...
    <div class="page-header">
        ...
        <div style="display:flex;gap:10px;align-items:center;">
            <a href="{{ route('clientes.importar') }}" class="btn-import">
    ...
    {{-- STAT CHIPS --}}
    {{-- FIX: usar estado_contado (columna real en BD), no estado_contacto --}}
    <div class="stat-chips">
        ...
        <div class="chip">
            ...
            <div>
                {{-- CORREGIDO: estado_contado en vez de estado_contacto --}}
                <div class="chip-val" style="color:#065f46;">{{ $clientes->where('estado_contado','COMPLETO')->count() }}</div>
        ...
        <div class="chip">
            ...
            <div>
                {{-- CORREGIDO: estado_contado --}}
                <div class="chip-val" style="color:#92400e;">{{ $clientes->where('estado_contado','INCOMPLETO')->count() }}</div>
        ...
        <div class="chip">
            ...
            <div>
                {{-- CORREGIDO: estado_contado --}}
                <div class="chip-val" style="color:#64748b;">{{ $clientes->where('estado_contado','SIN_DATOS')->count() }}</div>
    ...
    <div class="card">
        ...
        <div class="search-bar">
            ...
            <select class="form-select" id="filterTipo" onchange="filtrarClientes()" style="width:auto;min-width:180px;">
            ...
            <table id="clientesTable">
                ...
                @forelse($clientes as $cliente)
                    @php
                        ...
                        $estadoClass  = 'estado-' . strtolower($cliente->estado_contado ?? 'sin_datos');
                        $tipoCliente  = $cliente->tipo_cliente ?? 'PERSONA JURIDICA';
                    @endphp
                    <tr
                        data-search="{{ strtolower($cliente->razon_social . ' ' . $cliente->ruc) }}"
                        data-estado="{{ $cliente->estado_contado }}"
                        ...
                        <td>
                            @if($tipoCliente === 'PERSONA NATURAL')
                                <span class="tipo-badge tipo-natural">Persona Natural</span>
                            @else
                                <span class="tipo-badge tipo-juridica">Persona Jurídica</span>
                            @endif
                        </td>
                        ...
                        <td>
                            ...
                            <span class="estado-badge {{ $estadoClass }}">
                                {{ str_replace('_', ' ', $cliente->estado_contado ?? 'SIN DATOS') }}
                            </span>
                        </td>
                    ...
                    <tr>
                        <td colspan="7">
                            <div class="empty-state">
                                ...
                                <p style="font-size:13px;margin-top:4px;">Agrega tu primer cliente o importa desde Excel.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
